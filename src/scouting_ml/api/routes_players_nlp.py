...
EXPERIMENTAL_NLP_DISABLED_DETAIL = (
    "Experimental NLP routes are disabled. Set SCOUTING_ENABLE_EXPERIMENTAL_NLP_ROUTES=1 to enable them."
)


def _ensure_experimental_nlp_routes_enabled() -> None:
    if not load_api_runtime_config().experimental_nlp_routes:
        raise HTTPException(
            status_code=HTTP_503_SERVICE_UNAVAILABLE,
            detail=EXPERIMENTAL_NLP_DISABLED_DETAIL,
        )
...
@router.get(
    ...
    summary="Experimental: generate a scouting report for a player",
)
async def scouting_report(player_id: str) -> ScoutingReportResponse:
    """Return a human-readable scouting report generated by the NLP summarizer."""
    _ensure_experimental_nlp_routes_enabled()
    try:
        ...
        raise HTTPException(
            status_code=HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"Experimental NLP dependencies are unavailable: {exc}",
        ) from exc
...
@router.get(
    ...
    summary="Experimental: find similar players",
)
async def similar_players(player_id: str, top_k: int = Query(5, ge=1, le=50)) -> List[SimilarPlayer]:
...
@router.get(
    ...
    summary="Experimental: classify player role via zero-shot NLP",
)
async def player_role(player_id: str) -> RoleResponse:
